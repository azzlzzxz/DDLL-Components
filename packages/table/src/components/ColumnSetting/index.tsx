...
import type { FXColumns } from '../../typing';
...
const CheckboxList: React.FC<{
  list: (FXColumns<any> & { index?: number })[];
  className?: string;
  title: string;
  draggable: boolean;
  checkable: boolean;
...
const GroupCheckboxList: React.FC<{
  localColumns: (FXColumns<any> & { index?: number })[];
  className?: string;
  draggable: boolean;
  checkable: boolean;
  listsHeight?: number;
  ...
  const { hashId } = useContext(ProProvider);
  const rightList: (FXColumns<any> & { index?: number })[] = [];
  const leftList: (FXColumns<any> & { index?: number })[] = [];
  const list: (FXColumns<any> & { index?: number })[] = [];
  const intl = useIntl();
