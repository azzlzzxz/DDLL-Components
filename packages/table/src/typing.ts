...
export type ExtraFXColumnType<T> = Omit<
  ColumnType<T>,
...
export type FXColumnType<T = unknown, ValueType = 'text'> = ProSchema<
  T,
  ExtraFXColumnType<T> & {
    index?: number;
    /**
...
export type FXColumnGroupType<RecordType, ValueType> = {
  children: FXColumns<RecordType>[];
} & FXColumnType<RecordType, ValueType>;

export type FXColumns<T = any, ValueType = 'text'> =
  | FXColumnGroupType<T, ValueType>
  | FXColumnType<T, ValueType>;
...
export type FXTableProps<T, U, ValueType = 'text'> = {
  /**
   * @name 列配置能力，支持一个数组
   */
  columns?: FXColumns<T, ValueType>[];
  /**
   ...
   */
  tableRender?: (
    props: FXTableProps<T, U, ValueType>,
